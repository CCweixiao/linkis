...
public class HBaseShellSession implements ShellSession {
    ...
    public Result execute(String cmd) {
        if (!this.isConnected()) {
            String errorMsg = String.format("The current hbase shell session [%s] is not connected successfully," +
                    " please try again.", this);
            return Result.failed(errorMsg, new ExecutorInitException(HBASE_SHELL_ENV_INIT_FAILED.getErrorCode(),
                    HBASE_SHELL_ENV_INIT_FAILED.getErrorDesc()));
        }
